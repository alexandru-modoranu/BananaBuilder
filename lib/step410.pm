...
sub executeGoal
{
    ...
    if (exists $stepHash{'ASM_SOURCES'})
    {
        @asmSources = split(/\s+/, $stepHash{'ASM_SOURCES'});
    }
    else
    {
        logError("No ASM sources defined.");
    }

    logInfo("$stepName - Starting assembling for [ASM_SOURCES]\n");

    {
        local(*INFILE, $/);
        open(INFILE, "<", "$cwd/obj/asopt.cfg") or logError("Cannot open file $cwd/obj/asopt.cfg");
        $options = <INFILE>;
    }
    ...
    buildFileIndex(\@asmSources);

    chdir "obj";
    foreach my $file (@asmSources)
    {
        ...
        logInfo("Assembling $noExtFile\.o ...");
